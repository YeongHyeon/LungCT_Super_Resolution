...
class SRNET(object):
    ...
    def __init__(self):
        ...
        self.conv4 = tf.nn.relu(tf.add(tf.nn.conv2d(self.conv3, self.weights['w4'], strides=[1, 1, 1, 1], padding='SAME'), self.biases['b4']))
        ...
        self.optloss = self.loss + tf.exp(-self.psnr)
        self.optimizer = tf.train.AdamOptimizer(learning_rate=1e-4).minimize(loss=self.optloss)
